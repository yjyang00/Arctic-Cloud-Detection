...
acc.val.syst = sum(image.syst.val$label==-1)/length(image.syst.val$label)

# accuracy on validation set (2nd split)
idx = sample(unique(image.buf.train$fold), size = 1)
image.buf.val = image.buf.train[image.buf.train$fold==idx,]

acc.val.buf = sum(image.buf.val$label==-1)/length(image.buf.val$label)

# accuracy on test set (1st split)
acc.test.syst = sum(image.syst.test$label==-1)/length(image.syst.test$label)

# accuracy on test set (2nd split)
acc.test.buf = sum(image.buf.test$label==-1)/length(image.buf.test$label)
...
## FIRST SPLIT (systematic assignment):
test set: image.syst.test
train set & val set (10 folds): image.syst.train

## SECOND SPLIT (buffer):
test set: image.buf.test
train set & val set (12 folds): image.buf.train

In Baseline, one fold is RANDOMLY selected from the training set for VALIDATION. That is why you will see datasets named as `image.syst.val` and `image.buf.val`.
