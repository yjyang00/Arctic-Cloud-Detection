...
for (i in 1:2){
  ...
  image.test = rbind(image.test, image.syst[image.syst$fold==idx, ])
  idx.test = c(idx.test, idx)
}

# create training data
image.train = image.syst[!(image.syst$fold %in% idx.test), ]

table(image.test$fold)
table(image.train$fold)
```
